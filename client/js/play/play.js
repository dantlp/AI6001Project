import { apiFetch } from "../network/api.js";
import { createInputController } from "./input.js";
import { nextLivingIndexForTeam, weaponCatalog } from "./entities.js";
...
export function startApp() {
  input = createInputController({
    ...
    canCaptureGameInput: () => state.screen === "game" && Boolean(state.players.length)
  });
  setupUI();
  resizeCanvas();
...
function initializeMatch(online) {
  ...
  state.chargePower = 0;
  state.selectedWeapon = "shot";
  state.matchOver = false;
  state.aiThinking = 0;
  state.turnEnding = false;
  state.pendingTurnMessage = "";
  state.gameStarted = true;
  state.turnCursor = { player: -1, enemy: -1 };
  settlePlayers();
  showScreen("game");
...
function applySnapshot(match, fromRemote = false) {
  ...
  state.chargePower = 0;
  state.charging = false;
  state.selectedWeapon = match.selectedWeapon || "shot";
  state.aiThinking = 0;
  state.turnEnding = false;
  state.pendingTurnMessage = "";
  ...
  state.gameStarted = true;
  state.turnCursor = match.turnCursor || inferTurnCursorFromState();
  if (state.mode === "online") {
...
function inferTurnCursorFromState() {
  const cursor = { player: -1, enemy: -1 };
  if (state.players[state.activeIndex]?.alive) {
    cursor[state.activeTeam] = state.activeIndex;
  }
  return cursor;
}
...
function updateCurrentActor(step) {
  ...
  let moved = false;
  if (input.keys.a) { actor.x -= moveSpeed; actor.facing = -1; moved = true; }
  if (input.keys.d) { actor.x += moveSpeed; actor.facing = 1; moved = true; }
  ...
  if (input.keys.w && actor.jumpReady && isGrounded(actor)) {
    actor.vy = -6.2;
  ...
  if (!input.keys.w && isGrounded(actor)) actor.jumpReady = true;
  if (input.keys.s && !isGrounded(actor)) { actor.vy += 0.2 * step; markStateDirty(); }
}
...
function handleAI(actor, step) {
  ...
  fireProjectile(actor, actor.aiPlan.power, actor.aiPlan.loft, "shot");
  actor.aiPlan = null;
  state.aiThinking = 0;
}
function updateProjectiles(step) {
  for (let i = state.projectiles.length - 1; i >= 0; i -= 1) {
    ...
    if (isTerrainAt(projectile.x, projectile.y)) {
      explode(projectile.x, projectile.y, "land", projectile.weaponId);
      state.projectiles.splice(i, 1);
    ...
    for (const alien of state.players) {
      ...
      if (distance <= alien.radius + projectile.radius) {
        explode(projectile.x, projectile.y, "alien", projectile.weaponId);
        state.projectiles.splice(i, 1);
...
function explode(x, y, impactType, weaponId = "shot") {
  const weapon = weaponCatalog[weaponId] || weaponCatalog.shot;
  const blastRadius = weapon.blastRadius;
  carveTerrain(x, y, blastRadius);
  playSound(impactType === "alien" ? "hitAlien" : weapon.sound === "grenade" ? "grenadeImpact" : "hitLand");
  for (const alien of state.players) {
    if (!alien.alive) continue;
    const distance = Math.hypot(alien.x - x, alien.y - y);
    if (distance > blastRadius + 40) continue;
    applyDamage(alien, Math.max(10, Math.round(78 - distance * 1.15 + weapon.damageBoost)));
    const push = Math.max(0.5, (blastRadius + 42 - distance) / 16);
  ...
  for (let i = 0; i < 30; i += 1) {
    state.particles.push({ x, y, vx: randomRange(-3.2, 3.2), vy: randomRange(-4.4, -0.8), life: randomRange(14, 30), color: weaponId === "grenade" ? "#ffb347" : i % 2 === 0 ? "#ffd68a" : impactType === "alien" ? "#ff7b7b" : "#ff9a5e" });
  }
  markStateDirty();
}
...
function advanceTurn() {
  ...
  if (current?.alive) {
    state.turnCursor[current.team] = state.activeIndex;
  }
  ...
  state.turnCursor[nextTeam] = nextIndex;
...
function nextLivingIndex(team) {
  return nextLivingIndexForTeam(state.players, team, state.turnCursor[team]);
}
...
function fireProjectile(actor, powerPercent, loftBias = BASE_LOFT, weaponId = state.selectedWeapon) {
  const weapon = weaponCatalog[weaponId] || weaponCatalog.shot;
  state.projectiles.push({
    ownerId: actor.id,
    weaponId: weapon.id,
    x: actor.x + actor.facing * (actor.radius + 8),
    y: actor.y - actor.radius * 0.4,
    vx: actor.facing * (weapon.speedBase + (powerPercent / POWER_MAX) * weapon.speedScale),
    vy: -(loftBias ?? weapon.loft),
    radius: weapon.radius
  });
  playSound(weapon.sound);
  setOverlay(`${actor.name} launched a ${weapon.label.toLowerCase()} with ${Math.round(powerPercent)}% power.`);
  markStateDirty();
}

function toggleWeapon() {
  if (!canControlCurrentAlien() || state.charging) return;
  state.selectedWeapon = state.selectedWeapon === "shot" ? "grenade" : "shot";
  updateHUD();
  setOverlay(`${weaponCatalog[state.selectedWeapon].label} selected.`);
}
...
function releaseCharge() {
  if (!state.charging || !canControlCurrentAlien()) return;
  const actor = getCurrentAlien();
  const weapon = weaponCatalog[state.selectedWeapon] || weaponCatalog.shot;
  fireProjectile(actor, Math.max(18, state.chargePower), weapon.loft, state.selectedWeapon);
  state.charging = false;
  state.chargePower = 0;
}
...
function drawTrajectory() {
  ...
  let vx = actor.facing * (weapon.speedBase + (Math.max(18, state.chargePower) / POWER_MAX) * weapon.speedScale);
  let vy = -weapon.loft;
  ctx.save();
...
function updateHUD() {
  ...
  if (dom.hudWeapon) dom.hudWeapon.textContent = weaponCatalog[state.selectedWeapon]?.label || "Shot";
...
function playSound(type) {
  ...
  const specs = {
    shoot: ["triangle", 300, 150, 0.05, 0.16],
    grenade: ["square", 180, 110, 0.07, 0.2],
    grenadeImpact: ["sawtooth", 140, 60, 0.08, 0.24],
    hitLand: ["sawtooth", 110, 55, 0.06, 0.22],
    hitAlien: ["square", 420, 180, 0.05, 0.18],
    fall: ["sine", 180, 90, 0.045, 0.12],
    beep: ["sine", 880, 760, 0.03, 0.09]
  };
